...
Triangle::Triangle(const Triangle& rhs) : Shape(rhs) {

    for (int i = 0; i < 3; i++) {
        relVertex[i].x = rhs.getVertex_x(i);
        relVertex[i].y = rhs.getVertex_y(i);
    }
}
...
float Triangle::getVertex_x(int index) const {
    return relVertex[index].x;
}

float Triangle::getVertex_y(int index) const {
    return relVertex[index].y;
}
