...
void addPoly(stringstream& linestream, ShapeArray& shapeArray) {
    ...
    t_point relVertices[maxVertices];
    float xcen, ycen;
    ...
    for (int i = 0; i < nPoint; i++) {
        relVertices[i].x = vertices[i].x - xcen;
        relVertices[i].y = vertices[i].y - ycen;
    }
    // Now decide how you want to store these points, and make a
    // Polygon. Uncomment line below to add the Polygon to the 
    // shapeArray database.
    Polygon* poly = new Polygon(name, colour, xcen, ycen, relVertices, nPoint);
